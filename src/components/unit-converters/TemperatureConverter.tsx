          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
            <div>
              ...
              <div className="text-3xl font-semibold mt-1 bg-clip-text text-transparent bg-gradient-to-r from-white to-purple-100">
                {result} °{toUnit === 'kelvin' ? 'K' : toUnit === 'celsius' ? 'C' : toUnit === 'fahrenheit' ? 'F' : 'R'}
              </div>
            </div>
              ...
              <div className="mt-1 text-indigo-200">
                {fromUnit !== toUnit && (
                  <>
                    ...
                    {fromUnit === 'rankine' && toUnit === 'kelvin' && 'K = °R × 5/9'}
                    {fromUnit === 'kelvin' && toUnit === 'rankine' && '°R = K × 9/5'}
                    {fromUnit === 'celsius' && toUnit === 'rankine' && '°R = (°C + 273.15) × 9/5'}
                    {fromUnit === 'rankine' && toUnit === 'celsius' && '°C = (°R × 5/9) - 273.15'}
                    {fromUnit === 'fahrenheit' && toUnit === 'rankine' && '°R = °F + 459.67'}
                    {fromUnit === 'rankine' && toUnit === 'fahrenheit' && '°F = °R - 459.67'}
                  </>
                )}
              </div>
            </div>
        ...
        <div className="mt-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="bg-gray-900 bg-opacity-40 p-4 rounded-lg border border-gray-800 hover:border-purple-900/30 transition-all duration-300">
              <p className="py-1">0°C = <span className="text-purple-300 font-medium">32°F</span> (Freezing point)</p>
              <p className="py-1">100°C = <span className="text-purple-300 font-medium">212°F</span> (Boiling point)</p>
              <p className="py-1">-273.15°C = <span className="text-purple-300 font-medium">0K</span> (Absolute zero)</p>
            </div>
            <div className="bg-gray-900 bg-opacity-40 p-4 rounded-lg border border-gray-800 hover:border-purple-900/30 transition-all duration-300">
              <p className="py-1">Room temperature:</p>
              <p className="py-1">20-25°C = <span className="text-purple-300 font-medium">68-77°F</span></p>
              <p className="py-1">Body temperature: 37°C = <span className="text-purple-300 font-medium">98.6°F</span></p>
            </div>
          </div>
        </div>
      </div>
