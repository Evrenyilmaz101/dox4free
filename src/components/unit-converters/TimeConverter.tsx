...
const TimeConverter: React.FC = () => {
  ...
  const units: Unit[] = [
    ...
    { 
      name: 'month', 
      toSeconds: (value) => value * 2629746,  // Average month (365.25/12 days)
      fromSeconds: (value) => value / 2629746,
      symbol: 'mo'
    },
    { 
      name: 'year', 
      toSeconds: (value) => value * 31556952,  // Average year (365.25 days)
    ...
    { 
      name: 'nanosecond', 
      toSeconds: (value) => value / 1000000000,
      fromSeconds: (value) => value * 1000000000,
      symbol: 'ns'
    }
  ];
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
            ...
            <div className="text-left md:text-right mt-4 md:mt-0">
              ...
              <div className="mt-1 text-indigo-200">
                {fromUnit !== toUnit && (
                  <>
                    {fromUnit === 'second' && toUnit === 'minute' && '1 min = 60 sec'}
                    {fromUnit === 'minute' && toUnit === 'second' && '1 min = 60 sec'}
                    {fromUnit === 'hour' && toUnit === 'minute' && '1 hr = 60 min'}
                    {fromUnit === 'minute' && toUnit === 'hour' && '1 hr = 60 min'}
                    {fromUnit === 'day' && toUnit === 'hour' && '1 day = 24 hr'}
                    {fromUnit === 'hour' && toUnit === 'day' && '1 day = 24 hr'}
                    {fromUnit === 'week' && toUnit === 'day' && '1 week = 7 days'}
                    {fromUnit === 'day' && toUnit === 'week' && '1 week = 7 days'}
                    {fromUnit === 'month' && toUnit === 'day' && '1 month ≈ 30.44 days'}
                    {fromUnit === 'day' && toUnit === 'month' && '1 month ≈ 30.44 days'}
                    {fromUnit === 'year' && toUnit === 'day' && '1 year = 365.25 days'}
                    {fromUnit === 'day' && toUnit === 'year' && '1 year = 365.25 days'}
                    {fromUnit === 'second' && toUnit === 'millisecond' && '1 sec = 1000 ms'}
                    {fromUnit === 'millisecond' && toUnit === 'second' && '1 sec = 1000 ms'}
                    {fromUnit === 'millisecond' && toUnit === 'microsecond' && '1 ms = 1000 μs'}
                    {fromUnit === 'microsecond' && toUnit === 'millisecond' && '1 ms = 1000 μs'}
                    {fromUnit === 'microsecond' && toUnit === 'nanosecond' && '1 μs = 1000 ns'}
                    {fromUnit === 'nanosecond' && toUnit === 'microsecond' && '1 μs = 1000 ns'}
                  </>
                )}
              </div>
            </div>
        ...
        <div className="mt-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="bg-gray-900 bg-opacity-40 p-4 rounded-lg border border-gray-800 hover:border-purple-900/30 transition-all duration-300">
              <p className="py-1">1 minute = <span className="text-purple-300 font-medium">60 seconds</span></p>
              <p className="py-1">1 hour = <span className="text-purple-300 font-medium">60 minutes</span></p>
              <p className="py-1">1 day = <span className="text-purple-300 font-medium">24 hours</span></p>
              <p className="py-1">1 week = <span className="text-purple-300 font-medium">7 days</span></p>
            </div>
            <div className="bg-gray-900 bg-opacity-40 p-4 rounded-lg border border-gray-800 hover:border-purple-900/30 transition-all duration-300">
              <p className="py-1">1 second = <span className="text-purple-300 font-medium">1,000 milliseconds</span></p>
              <p className="py-1">1 millisecond = <span className="text-purple-300 font-medium">1,000 microseconds</span></p>
              <p className="py-1">1 microsecond = <span className="text-purple-300 font-medium">1,000 nanoseconds</span></p>
              <p className="py-1">1 year ≈ <span className="text-purple-300 font-medium">365.25 days</span></p>
            </div>
          </div>
        </div>
      </div>
