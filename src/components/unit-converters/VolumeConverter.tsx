...
type Unit = {
  name: string;
  conversion: number; // Conversion rate to liters (base unit)
};
...
const VolumeConverter: React.FC = () => {
  ...
  const units: Unit[] = [
    { name: 'milliliter', conversion: 0.001 },
    { name: 'liter', conversion: 1 },
    { name: 'cubic meter', conversion: 1000 },
    { name: 'cubic centimeter', conversion: 0.001 },
    { name: 'cubic inch', conversion: 0.0163871 },
    { name: 'cubic foot', conversion: 28.3168 },
    { name: 'fluid ounce (US)', conversion: 0.0295735 },
    { name: 'cup (US)', conversion: 0.236588 },
    { name: 'pint (US)', conversion: 0.473176 },
    { name: 'quart (US)', conversion: 0.946353 },
    { name: 'gallon (US)', conversion: 3.78541 },
    { name: 'gallon (UK)', conversion: 4.54609 },
    { name: 'barrel (oil)', conversion: 158.987 },
  ];
  ...
  const convert = () => {
    ...
    setTimeout(() => {
      ...
      const liters = Number(inputValue) * fromUnitData.conversion;
      const convertedValue = liters / toUnitData.conversion;
      ...
      <div className="relative z-10">
        ...
        <div className={`mt-8 p-6 bg-gray-900 bg-opacity-70 rounded-lg border border-gray-800 transition-all duration-300 relative overflow-hidden ${isConverting ? 'opacity-70' : 'opacity-100'}`}>
          ...
          <div className="mt-4 text-sm text-purple-200">
            Convert between various volume units including metric, imperial, and cooking measurements
          </div>
        </div>
        
        {/* Common Conversions Reference */}
        <div className="mt-8 p-4 bg-gray-900 bg-opacity-50 rounded-lg border border-gray-800">
          <h3 className="text-lg font-medium text-purple-200 mb-3">Common Volume Conversions</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm text-gray-300">
            <div>• 1 gallon (US) = 3.785 liters</div>
            <div>• 1 gallon (UK) = 4.546 liters</div>
            <div>• 1 liter = 1000 milliliters</div>
            <div>• 1 cup (US) = 236.6 milliliters</div>
            <div>• 1 cubic foot = 28.32 liters</div>
            <div>• 1 barrel (oil) = 159 liters</div>
          </div>
        </div>
      </div>
    </div>
