...
type Unit = {
  name: string;
  conversion: number; // Conversion rate to meters per second (base unit)
};

const SpeedConverter: React.FC = () => {
  const [inputValue, setInputValue] = useState<string>('1');
  const [fromUnit, setFromUnit] = useState<string>('meter per second');
  ...
  const units: Unit[] = [
    { name: 'meter per second', conversion: 1 },
    { name: 'kilometer per hour', conversion: 0.277778 },
    { name: 'mile per hour', conversion: 0.44704 },
    { name: 'foot per second', conversion: 0.3048 },
    { name: 'knot', conversion: 0.514444 },
    { name: 'mile per minute', conversion: 26.8224 },
    { name: 'kilometer per minute', conversion: 16.6667 },
    { name: 'speed of light', conversion: 299792458 },
    { name: 'speed of sound', conversion: 343 },
    { name: 'mach', conversion: 343 },
  ];
  ...
  const convert = () => {
    ...
    setTimeout(() => {
      ...
      const metersPerSecond = Number(inputValue) * fromUnitData.conversion;
      const convertedValue = metersPerSecond / toUnitData.conversion;
      ...
      <div className="relative z-10">
        ...
        <div className={`mt-8 p-6 bg-gray-900 bg-opacity-70 rounded-lg border border-gray-800 transition-all duration-300 relative overflow-hidden ${isConverting ? 'opacity-70' : 'opacity-100'}`}>
          ...
          <div className="mt-4 text-sm text-purple-200">
            Convert between various speed and velocity units including metric, imperial, and specialized measurements
          </div>
        </div>
        
        {/* Common Conversions Reference */}
        <div className="mt-8 p-4 bg-gray-900 bg-opacity-50 rounded-lg border border-gray-800">
          <h3 className="text-lg font-medium text-purple-200 mb-3">Common Speed Conversions</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm text-gray-300">
            <div>• 1 mph = 1.609 km/h</div>
            <div>• 1 knot = 1.852 km/h</div>
            <div>• 1 m/s = 3.6 km/h</div>
            <div>• Mach 1 ≈ 343 m/s (at sea level)</div>
            <div>• Speed of light = 299,792,458 m/s</div>
            <div>• 60 mph = 88 feet per second</div>
          </div>
        </div>
      </div>
    </div>
