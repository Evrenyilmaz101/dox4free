...
type Unit = {
  name: string;
  conversion: number; // Conversion rate to pascals (base unit)
};
...
const PressureConverter: React.FC = () => {
  ...
  const units: Unit[] = [
    { name: 'pascal', conversion: 1 },
    { name: 'kilopascal', conversion: 1000 },
    { name: 'bar', conversion: 100000 },
    { name: 'millibar', conversion: 100 },
    { name: 'atmosphere', conversion: 101325 },
    { name: 'psi', conversion: 6894.76 },
    { name: 'torr', conversion: 133.322 },
    { name: 'mmHg', conversion: 133.322 },
    { name: 'inHg', conversion: 3386.39 },
    { name: 'kPa', conversion: 1000 },
  ];
  ...
  const convert = () => {
    ...
    setTimeout(() => {
      ...
      const pascals = Number(inputValue) * fromUnitData.conversion;
      const convertedValue = pascals / toUnitData.conversion;
      ...
      <div className="relative z-10">
        ...
        <div className={`mt-8 p-6 bg-gray-900 bg-opacity-70 rounded-lg border border-gray-800 transition-all duration-300 relative overflow-hidden ${isConverting ? 'opacity-70' : 'opacity-100'}`}>
          ...
          <div className="mt-4 text-sm text-purple-200">
            Convert between various pressure units including scientific, engineering, and atmospheric measurements
          </div>
        </div>
        
        {/* Common Conversions Reference */}
        <div className="mt-8 p-4 bg-gray-900 bg-opacity-50 rounded-lg border border-gray-800">
          <h3 className="text-lg font-medium text-purple-200 mb-3">Common Pressure Conversions</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm text-gray-300">
            <div>• 1 bar = 100,000 pascals</div>
            <div>• 1 atmosphere = 101,325 pascals</div>
            <div>• 1 psi = 6,895 pascals</div>
            <div>• 1 torr = 133.3 pascals</div>
            <div>• 1 mmHg = 133.3 pascals</div>
            <div>• 1 kPa = 1,000 pascals</div>
          </div>
        </div>
      </div>
    </div>
