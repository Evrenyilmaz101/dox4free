...
type Unit = {
  name: string;
  toSquareMeters: (value: number) => number; // Convert to square meters (base unit)
  fromSquareMeters: (value: number) => number; // Convert from square meters
  symbol: string;
};

const AreaConverter: React.FC = () => {
  const [inputValue, setInputValue] = useState<string>('1');
  const [fromUnit, setFromUnit] = useState<string>('square_meters');
  const [toUnit, setToUnit] = useState<string>('square_feet');
  const [result, setResult] = useState<string>('');
  const [error, setError] = useState<string>('');
  const [isConverting, setIsConverting] = useState<boolean>(false);

  // Define area units with their conversion factors
  const units: Unit[] = [
    {
      name: 'square_meters',
      toSquareMeters: (sqm) => sqm,
      fromSquareMeters: (sqm) => sqm,
      symbol: 'm²'
    },
    {
      name: 'square_feet',
      toSquareMeters: (sqft) => sqft * 0.09290304,
      fromSquareMeters: (sqm) => sqm / 0.09290304,
      symbol: 'ft²'
    },
    {
      name: 'square_inches',
      toSquareMeters: (sqin) => sqin * 0.00064516,
      fromSquareMeters: (sqm) => sqm / 0.00064516,
      symbol: 'in²'
    },
    {
      name: 'square_yards',
      toSquareMeters: (sqyd) => sqyd * 0.83612736,
      fromSquareMeters: (sqm) => sqm / 0.83612736,
      symbol: 'yd²'
    },
    {
      name: 'square_miles',
      toSquareMeters: (sqmi) => sqmi * 2589988.11,
      fromSquareMeters: (sqm) => sqm / 2589988.11,
      symbol: 'mi²'
    },
    {
      name: 'square_kilometers',
      toSquareMeters: (sqkm) => sqkm * 1000000,
      fromSquareMeters: (sqm) => sqm / 1000000,
      symbol: 'km²'
    },
    {
      name: 'hectares',
      toSquareMeters: (ha) => ha * 10000,
      fromSquareMeters: (sqm) => sqm / 10000,
      symbol: 'ha'
    },
    {
      name: 'acres',
      toSquareMeters: (acre) => acre * 4046.86,
      fromSquareMeters: (sqm) => sqm / 4046.86,
      symbol: 'ac'
    }
  ];
  ...
  const convert = () => {
    ...
    setTimeout(() => {
      ...
      try {
        // First convert to square meters, then to target unit
        const sqMeters = fromUnitData.toSquareMeters(Number(inputValue));
        const convertedValue = toUnitData.fromSquareMeters(sqMeters);
        ...
        <div className="mt-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="bg-gray-900 bg-opacity-40 p-4 rounded-lg border border-gray-800 hover:border-purple-900/30 transition-all duration-300">
              ...
              <p className="py-1">• 1 square meter (m²) - base unit</p>
              <p className="py-1">• 1 square kilometer (km²) = 1,000,000 m²</p>
              <p className="py-1">• 1 hectare (ha) = 10,000 m²</p>
            </div>
            <div className="bg-gray-900 bg-opacity-40 p-4 rounded-lg border border-gray-800 hover:border-purple-900/30 transition-all duration-300">
              <p className="py-1"><span className="text-purple-300 font-medium">Imperial/US Units:</span></p>
              <p className="py-1">• 1 square foot (ft²) = 0.0929 m²</p>
              <p className="py-1">• 1 square yard (yd²) = 0.8361 m²</p>
              <p className="py-1">• 1 acre (ac) = 4,047 m²</p>
              <p className="py-1">• 1 square mile (mi²) = 2,590,000 m²</p>
            </div>
          </div>
          
          <div className="mt-6 bg-gray-900 bg-opacity-40 p-4 rounded-lg border border-gray-800">
            <p className="text-sm">
              <span className="text-purple-300 font-medium">Common Conversions:</span>
            </p>
            <p className="text-sm pt-2">• 1 square meter = 10.764 square feet</p>
            <p className="text-sm pt-1">• 1 hectare = 2.471 acres</p>
            <p className="text-sm pt-1">• 1 square kilometer = 0.386 square miles</p>
            <p className="text-sm pt-1">• 1 acre = 4,840 square yards = 43,560 square feet</p>
            <p className="text-sm pt-1">• 1 square mile = 640 acres</p>
          </div>
        </div>
      </div>
    </div>
